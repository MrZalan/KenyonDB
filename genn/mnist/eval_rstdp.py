...
kc_mbon_g = np.load("kc_mbon_g.npy").astype(np.float32)
...
g = kc_mbon.vars["g"].view
print("kc_mbon.g stats:", float(g.min()), float(g.max()), float(g.mean()))

def decode_earliest(mbon_spike_times, mbon_spike_ids):
    return int(mbon_spike_ids[np.argmin(mbon_spike_times)])

def decode_count(mbon_spike_ids):
    # spike count per class
    counts = np.bincount(mbon_spike_ids.astype(np.int64), minlength=NUM_MBON)
    return int(np.argmax(counts)), counts

def decode_earliest_tiebreak(mbon_spike_times, mbon_spike_ids):
    # earliest class, but if multiple classes share same earliest time (rare), break by spike count
    t0 = np.min(mbon_spike_times)
    first_classes = mbon_spike_ids[mbon_spike_times == t0]
    if len(first_classes) == 1:
        return int(first_classes[0])
    pred_count, _ = decode_count(mbon_spike_ids)
...
for s in range(4):
    ...
    pred_e = decode_earliest(mbon_spike_times, mbon_spike_ids)
    pred_c, counts = decode_count(mbon_spike_ids)

    axes[2].axhline(pred_e, linestyle="--", color="red", alpha=0.3)
    axes[2].axhline(pred_c, linestyle=":", color="orange", alpha=0.5)

    axes[2].set_ylabel("MBON")
    axes[2].set_xlabel("Time [ms]")

    print(f"Sample {s}: label={testing_labels[s]} earliest={pred_e} count={pred_c} "
          f"spikes={len(mbon_spike_times)} counts={counts}")

    model.custom_update("reset_group")

# --- Full evaluation ---
#num_correct_earliest = 0
#num_correct_count = 0
#num_correct_tiebreak = 0
#num_spiking = 0
num_correct = 0
...
for s in tqdm(range(testing_images.shape[0])):
    ...
    if len(mbon_spike_times) > 0:
     #   num_spiking += 1
      #  pred_e = decode_earliest(mbon_spike_times, mbon_spike_ids)
       # pred_c, _ = decode_count(mbon_spike_ids)
    #    pred_t = decode_earliest_tiebreak(mbon_spike_times, mbon_spike_ids)

     #   y = int(testing_labels[s])
      #  num_correct_earliest += (pred_e == y)
       # num_correct_count += (pred_c == y)
        #num_correct_tiebreak += (pred_t == y)
...
#N = testing_images.shape[0]
#print(f"\nEarliest spike: {num_correct_earliest}/{N} correct ({(num_correct_earliest*100.0)/N:.2f} %)")
#print(f"Spike count:    {num_correct_count}/{N} correct ({(num_correct_count*100.0)/N:.2f} %)")
#print(f"Earliest+tiebk: {num_correct_tiebreak}/{N} correct ({(num_correct_tiebreak*100.0)/N:.2f} %)")
#print(f"MBON spiked on {num_spiking}/{N} samples ({(num_spiking*100.0)/N:.2f} %)")
