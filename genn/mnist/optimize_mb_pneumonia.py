...
STUDY_NAME = "mushroom_body_pneumonia"
N_TRIALS = 150 
CHECKPOINT_DIR = "checkpoints_mb_pneumonia"
...
def objective(trial):
    ...
    with mlflow.start_run(run_name=f"Trial_{trial.number}", nested=True):
        ...
        try:
            # MedMNIST betöltése
            train_imgs, train_labels, test_imgs, test_labels, num_classes = load_medmnist('pneumoniamnist')
            ...
            if is_best:
                np.save(os.path.join(CHECKPOINT_DIR, "best_weights.npy"), trained_weights)
                np.save(os.path.join(CHECKPOINT_DIR, "best_indices.npy"), trained_indices)
                mlflow.log_dict(params_medmnist, "best_params.json") # BUG FIX: params_medmnist
...
if __name__ == "__main__":
    ...
    print(f"Starting search across {N_TRIALS} trials on full pneumoniamnist.")
